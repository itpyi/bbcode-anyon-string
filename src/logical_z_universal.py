...
"""
Universal (matrix-based) construction of BB logical Z operators.
...
def _groebner_row_space(
    ...
    basis_matrix = row_basis_uint8(stacked)
...
def _ann_intersection_basis(
    ...
    A = to_uint8_matrix(ann_matrix)
    P = to_uint8_matrix(principal_matrix)
    ...
    W = to_uint8_matrix(mod2.nullspace(M))
    if W.size == 0:
    ...
    B = row_basis_uint8(np.vstack(inter_rows).astype(np.uint8))
...
def compute_ann_f_matrix(f_poly, monomials, l: int, m: int):
    ...
    for vec in nullspace_vecs:
        coeffs = to_uint8_array(vec).reshape(-1)
...
def compute_tor_2(
    ...
    f_ann_g_matrix = to_uint8_matrix(ann_g_quotient["M_g"])
    g_ann_f_matrix = to_uint8_matrix(ann_f_quotient["M_g"])
...
def compute_logical_z_universal(
    f_str: str,
    g_str: str,
    l: int,
    m: int,
...
def verify_logical_z_equivalence(
    ...
    z_stab_basis = row_basis_uint8(Hz, num_cols=Hz.shape[1])
...
def verify_logical_x_equivalence(
    ...
    x_stab_basis = row_basis_uint8(Hx, num_cols=Hx.shape[1])
